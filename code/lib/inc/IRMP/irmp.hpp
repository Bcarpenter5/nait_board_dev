...
// turn this off it you want to live
#define MY_MICRO
...
#if defined(__MBED__)
// DigitalIn Pin(IRMP_PIN, PullUp);                                // this requires mbed.h and source to be compiled as cpp
...
#  endif
#endif
#if defined (ARM_STM32_HAL)                                           // STM32 with Hal Library: do nothing
...
#  endif
  ...
  #include "gpio.h"
  //already setup in the main file ??
...
#endif
   volatile uint_fast32_t           irmp_input;                                             // input value
...
#else
    // uint_fast8_t val = GPIOA->IDR;
#if defined(MY_MICRO)
    
    irmp_input = (GPIOB->IDR & (1 << 8));
#else
    irmp_input = (GPIOA->IDR & (1 << 8));
    ...
    //irmp_input = 1;//(gpio_read(GPIOA,8));
    //printf("%d\n",irmp_input);
   //__asm ("ldr r0, =0x50000010 - .\n" // Load the immediate address into r0
   //"ldr r0, [r0]\n"
   //"movs r1, #1\n"
   //"lsls r1, r1, #8\n"
   //"ands r0, r1\n"
