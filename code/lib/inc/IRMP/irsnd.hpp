...
static void /*************************************************************************** set frequency ************************************************************************************/
irsnd_set_freq (int freq)
{
        int period = SystemCoreClock/freq;
        PWM_Init(TIM17,period-1,period/2-1);
}
...
irsnd_init (void)
{
     // initialize the PWM  and the timer and the pin 
        RCC->APBENR2 |= RCC_APBENR2_TIM17EN; // enable the clock for the timer
        GPIO_InitAlternateF(GPIOB, 9, 2);// set it two chanel 17
        irsnd_set_freq (36000);                                         // set default frequency
}
